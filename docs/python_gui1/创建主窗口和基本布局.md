...
接下来重点实现第2部分的内容，上一步我们已经实现了表头的内容，接下来定义一些表格中的数据，测试下显示的效果，实际中这部分数据从数据库中获取。给`StudentInterface`定义一个实例例方法`load_data`用来加载数据，并且在`__init__`中初始化。创建实例方法广`popualte_table`给表格填充数据。
